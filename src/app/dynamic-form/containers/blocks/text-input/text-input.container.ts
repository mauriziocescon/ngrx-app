...
export class TextInputContainerComponent {
  ...
  protected dispatchValueDidChangeAction(value: string): void {
    ...
    if (this.textInputBlock.minLength >= 0 && value.length < this.textInputBlock.minLength) {
      valid = false;
    }

    if (this.textInputBlock.maxLength >= 0 && value.length > this.textInputBlock.maxLength) {
      valid = false;
    }
    const block = {
      block: {
